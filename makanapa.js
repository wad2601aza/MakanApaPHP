// --- CONFIGURATION ---
// API_BASE is defined in makanapa_api.js — edit it there.
...
async function sendRequest(text) {
    ...
    try {
        const req = await API.createRequest(currentDbUser?.id || null, finalBuyerDisplay, text, 1);
...
async function renderHabits() {
    try {
        ...
        if (!habits || (!habits.avg_price && !habits.last_food && !habits.total_orders)) {
...
async function submitOffer(reqId) {
    ...
    // Build FormData for PHP multipart endpoint (handles file upload natively)
...
async function saveUserHabit(food, price, isCheapest) {
    ...
    try {
        ...
        const newTotalOrders   = (habit?.total_orders   || 0) + 1;
        const newCheapestCount = (habit?.cheapest_count || 0) + (isCheapest ? 1 : 0);
        const prevAvg          = habit?.avg_price || price;
        const newAvg           = Math.round(((prevAvg * (newTotalOrders - 1)) + price) / newTotalOrders);
        await API.saveHabits(currentDbUser.id, food, newAvg, newTotalOrders, newCheapestCount);
    } catch (err) {
...
async function submitOrder() {
    ...
    try {
        // Single atomic PHP call: deducts balance + inserts order + decrements stock
        const result = await API.placeOrder({
            request_id:      currentRequestId,
            user_id:         currentDbUser.id,
            offer_id:        currentOrderContext.offerId || null,
...
async function loadOrderHistory() {
    ...
    try {
        ...
        // Fallback: if empty, try phone-suffix search (catches older records)
        if ((!orders || orders.length === 0) && phoneSuffix) {
            const fallback = await API.getOrdersByBuyer(phoneSuffix);
...
async function cancelOrder(orderId) {
    ...
    try {
        // Single PHP call handles: fetch order → refund balance → cancel → restore stock
        const result = await API.cancelOrder(orderId);
        // If the currently logged-in buyer is the one being refunded, sync local balance
        if (currentDbUser && result.new_balance !== null && result.new_balance !== undefined) {
            currentDbUser.balance = result.new_balance;
            loadBalance();
        }
        showToast('Order Cancelled 🚫', 'The money has been refunded to the buyer.', 'success');
        if (typeof isSellerPage !== 'undefined' && isSellerPage) loadSellerHistory();
