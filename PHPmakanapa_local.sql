-- ============================================================
-- MakanApa — COMPLETE SCHEMA
-- Works on: XAMPP MariaDB Port 3307  AND  InfinityFree Port 3306
-- Run in phpMyAdmin → select your DB → SQL tab → paste → Go
--
-- SAFE: drops & recreates all tables (clears existing data).
-- If you want to keep data, use the ALTER version below.
-- ============================================================
...
-- ── 4. ORDERS ────────────────────────────────────────────────
-- status values: 'pending' | 'on process' | 'completed' | 'cancelled'
-- is_rated: 0 = not yet rated, 1 = buyer has submitted a rating
CREATE TABLE `orders` (
  `id`              INT(11)      NOT NULL AUTO_INCREMENT,
  `user_id`         INT(11)      DEFAULT NULL,
...
-- ============================================================
-- SAFE ALTER VERSION — run this instead if you have live data
-- ============================================================
-- ALTER TABLE `users`
...
--   `seller_id` INT(11) DEFAULT NULL, `buyer_id` INT(11) DEFAULT NULL,
--   `stars` TINYINT(1) NOT NULL DEFAULT 5, `comment` TEXT DEFAULT NULL,
--   `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
--   PRIMARY KEY (`id`), UNIQUE KEY `uq_rating_order` (`order_id`)
-- ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
